...
from .aggregators_tf2 import MyMeanAggregator, MaxPoolAggregator
import numpy as np
...
class MGCCF(object):
    def __init__(self, load_checkpoint, dataset_argv, architect_argv, adj_degree, num_samples,
        ...
        with self.graph.as_default():
            ...
            self.old_u_i_adj_mat = tf.compat.v1.sparse.placeholder(tf.int32, shape=[self.old_num_user, self.old_num_item],
                                                         name='ui_1hop_mat_ph')
            self.old_i_u_adj_mat = tf.compat.v1.sparse.placeholder(tf.int32, shape=[self.old_num_item, self.old_num_user],
                                                         name='iu_1hop_mat_ph')
            ...
            self.old_item_gs = tf.compat.v1.placeholder(tf.float64,
                                              ...
                                              name='u_old_gs_ph')
            self.old_u_cluster_mat = tf.compat.v1.sparse.placeholder(tf.int32, shape=[self.k_centroids[0], self.old_num_user],
                                                           name='old_u_cluster_mat_ph')
            self.old_i_cluster_mat = tf.compat.v1.sparse.placeholder(tf.int32, shape=[self.k_centroids[1], self.old_num_item],
    ...
    def model_fn(self, scope):
        ...
        with tf.compat.v1.variable_scope(scope):
            if self.pretrain_data is None:
                self.user_embeddings = tf.Variable(initializer(shape=[self.num_user, self.input_dim], dtype=tf.float64), name='user_embedding')
                self.item_embeddings = tf.Variable(initializer(shape=[self.num_item, self.input_dim], dtype=tf.float64), name='item_embedding')
            ...
            self.user_medium_input_1 = tf.Variable(initializer(shape=[self.num_item, self.layer_dims[1]], dtype=tf.float64), name='user_medium_input_1')
            self.item_medium_input_1 = tf.Variable(initializer(shape=[self.num_user, self.layer_dims[1]], dtype=tf.float64), name='item_medium_input_1')
            self.user_medium_input_2 = tf.Variable(initializer(shape=[self.num_user, self.layer_dims[2]], dtype=tf.float64), name='user_medium_input_2')
            self.item_medium_input_2 = tf.Variable(initializer(shape=[self.num_item, self.layer_dims[2]], dtype=tf.float64), name='item_medium_input_2')
