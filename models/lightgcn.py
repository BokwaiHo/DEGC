...
from .aggregators import MyMeanAggregator, MaxPoolAggregator, LightGCNAggregator, LightGCNAggregator_Raw
import numpy as np

"""
TensorFlow Implementation for "LightGCN: Simplifying and Powering Graph Convolution Network for Recommendation"
__author__: Bowei He
__date__: Aug. 2022
"""
...
class LightGCN(object):
    def __init__(self, load_checkpoint, dataset_argv, architect_argv, adj_degree, num_samples,
                 ptmzr_argv, batch_size, neigh_drop_rate, l2_embed,
                 neg_item_num=10, ui_con_positive=15, con_ratios = [1,1,1,1,1,1,1,1], pretrain_data=None,
                 inc_reg=[0, 0, 0], old_num_user=0, old_num_item=0, distill_mode='',k_centroids=[0, 0], tau=0,
        ...
        with self.graph.as_default():
            ...
            for i in range(1, self.num_layers):
                    self.user_agg_funcs.append(LightGCNAggregator(self.load_checkpoint, 'user_graph_agg_%d' % i, self.num_samples[::-1][i - 1],
                                                                self.input_dim, self.layer_dims[i - 1],
                                                                self.layer_dims[i],
                                                                dropout=neigh_drop_rate))
                    self.item_agg_funcs.append(LightGCNAggregator(self.load_checkpoint, 'item_graph_agg_%d' % i, self.num_samples[::-1][i - 1],
                                                                self.input_dim, self.layer_dims[i - 1],
                                                                self.layer_dims[i],
                                                                dropout=neigh_drop_rate))
    ...
    def model_fn(self, scope):
        ...
        with tf.compat.v1.variable_scope(scope):
            ...
            user_rep, user_embed, self.user_medium_input_1, self.user_medium_input_2 = self.graphconv('user_gcn', self.u_id,
                                                          self.user_embeddings, self.item_embeddings, 'user', True)


            user_rep = tf.concat([user_rep, user_embed], 1)

            user_rep = tf.expand_dims(user_rep, 1)


            # ================================================================
            pos_item_rep, pos_item_embed, self.item_medium_input_1, self.item_medium_input_2 = self.graphconv('pos_item_gcn',
                                                                  self.pos_item_id,
                                                                  self.user_embeddings, self.item_embeddings,
                                                                  'pos_item', True)

            pos_item_rep = tf.concat([pos_item_rep, pos_item_embed], 1)
        ...
        return bpr_loss, reg_loss, tf.constant(0, dtype=tf.float64), contrastive_loss
