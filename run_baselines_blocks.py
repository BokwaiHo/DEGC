...
def generate_local_neighbor(u_adj_list, i_emb, distill_mode, num_neigh):
    ...
    for u, i_list in u_adj_list.items():
        ...
        if len(i_list) > 0:
            ...
            u_ls = softmax(u_i_distance)  # 1 * num_neigh #shape (num_neigh, 0)
        ...
        while _epoch <= n_epoch:
            ...
            for iter in range(0, num_iter):
                ...
                print('Epoch ', '%04d' % _epoch, 'iter ', '%02d' % iter,
                      ...
                      'contrastive_loss= {:.5f},cost {:.4f} seconds'.
                      format(contrastive_loss, time.time() - iter_start))
...
if __name__ == '__main__':
    ...
    save_setting = f"cp{parser.ui_con_positive}cr{save_ratio}lc{save_lambda_con}" + \
